...
// 将非序列化的SM2公钥字节数组转换为椭圆曲线库标准的公钥
func UnmarshalSM2Pubkey(pub []byte) (*sm2.PublicKey, error) {
	x, y := elliptic.Unmarshal(Sm2p256(), pub)
	if x == nil {
		return nil, errInvalidPubkey
	}
	return &sm2.PublicKey{Curve: Sm2p256(), X: x, Y: y}, nil
}
...
// 通过SM2公钥信息和椭圆曲线参数生成指定长度字节数组的算法
func FromSM2Pub(pub *sm2.PublicKey) []byte {
	if pub == nil || pub.X == nil || pub.Y == nil {
		return nil
	}
	return elliptic.Marshal(Sm2p256(), pub.X, pub.Y)
}
...
func Sm2PubkeyToAddress(p sm2.PublicKey) common.Address {
	pubBytes := FromSM2Pub(&p)
	//TODO: 后面将keccak256算法替换为SM3
	return common.BytesToAddress(Keccak256(pubBytes[1:])[12:])
}

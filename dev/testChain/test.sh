...
./build/bin/geth --identity $identity --rpc --rpcport "8549" --rpccorsdomain "http://localhost:8084" --rpcapi $rpcAPI --datadir "$testDataDir"/node4 --ipcpath "$testDataDir"/node4/geth.ipc --port "30307" --ipcpath "node4.rpc" --nodiscover --allow-insecure-unlock --regulatorip $regulatorIP --exchangeip $exchangeIP 2>>"$testDataDir"/node4/log.log&